...
def _fallback_result(item: EnrichedLineItem, cbr_results: dict, ml_results: dict) -> LLMPass1Result:
    ...
    # Build a data-driven fallback reason rather than a generic placeholder
...
async def process_batch(
    ...
    # Map results back by account_code, fallback for any missing
...
async def run_pass1(
    ...
    """
    # Load SOP rules from DB
    sop_rows = db.execute(
        "SELECT rule_text FROM sop_rules WHERE active = 1 ORDER BY id"
    ).fetchall()
    sop_rules = [r[0] for r in sop_rows]

    # Filter to non-readOnly items only
