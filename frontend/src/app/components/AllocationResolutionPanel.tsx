...
import { issueAnchor, slicesBalance } from '../lib/allocationResolution';
...
export function AllocationResolutionPanel({ hoaId }: Props) {
  ...
  const [reason, setReason] = useState('Operator confirmed executable basis from source evidence');
  const [methodByPool, setMethodByPool] = useState<Record<string, string>>({});
  const [splitLabel, setSplitLabel] = useState('Electricity & Gas');
  const [splitAmount, setSplitAmount] = useState('16800');
  const [splitLeft, setSplitLeft] = useState('5600');
  const [splitRight, setSplitRight] = useState('11200');
  const [leftPool, setLeftPool] = useState('variable_dre_exceptions');
  const [rightPool, setRightPool] = useState('equal_base');
  const [leftCategory, setLeftCategory] = useState('gas');
  const [rightCategory, setRightCategory] = useState('electricity');
  ...
  const splitDelta = slicesBalance(Number(splitAmount), [
  ...
  return (
    <section id="allocation-resolution" className="space-y-4 rounded-lg border border-[#e5e5e5] bg-white p-5">
      ...
      <div id={issueAnchor('line:electricity gas')} className="rounded-lg border border-[#eeeeee] p-4">
        <h3 className="font-medium text-[#111111]">Combined-line splitter</h3>
        <p className="mt-1 text-sm text-[#525252]">
          The original source amount is immutable. Slices must sum to that amount.
        </p>
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <label className="text-xs text-[#525252]">
            Source line
            <input className="mt-1 w-full rounded-md border px-2 py-1.5 text-sm" value={splitLabel} onChange={(e) => setSplitLabel(e.target.value)} />
          </label>
          <label className="text-xs text-[#525252]">
            Source annual
            <input className="mt-1 w-full rounded-md border px-2 py-1.5 text-sm" value={splitAmount} onChange={(e) => setSplitAmount(e.target.value)} />
          </label>
          <label className="text-xs text-[#525252]">
            First slice ({leftCategory})
            <input className="mt-1 w-full rounded-md border px-2 py-1.5 text-sm" value={splitLeft} onChange={(e) => setSplitLeft(e.target.value)} />
          </label>
          <label className="text-xs text-[#525252]">
            Residual slice ({rightCategory})
            <input className="mt-1 w-full rounded-md border px-2 py-1.5 text-sm" value={splitRight} onChange={(e) => setSplitRight(e.target.value)} />
          </label>
        </div>
        <p className="mt-2 text-xs text-[#737373]">
          {splitDelta === 0 ? 'Slices balance the source line.' : `Slices are off by ${splitDelta}.`}
        </p>
        <button
          type="button"
          className="mt-2 rounded-md border border-[#d4d4d4] px-3 py-1.5 text-sm"
          disabled={splitDelta !== 0 || busy !== null}
          onClick={async () => {
            setBusy('slices');
            try {
      ...
      <div className="rounded-lg border border-dashed border-[#d4d4d4] bg-[#fafafa] p-4">
        <h3 className="font-medium text-[#111111]">Non-final prior vs proposed preview</h3>
        <p className="mt-1 text-xs uppercase tracking-wide text-[#737373]">Preview only — not a final package</p>
        {preview ? (
          <pre className="mt-3 max-h-64 overflow-auto text-xs text-[#333333]">
            {JSON.stringify((preview as { preview?: unknown }).preview ?? preview, null, 2)}
          </pre>
        ) : (
          <p className="mt-2 text-sm text-[#666666]">Preview appears after a draft method is saved.</p>
        )}
      </div>
